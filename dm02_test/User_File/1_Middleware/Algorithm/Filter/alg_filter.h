...
extern const float FOURIER_FILTER_DEFAULT_SAMPLING_FREQUENCY;
...
{
public:
    void Init(const float &__Value_Constrain_Low = 0.0f, const float &__Value_Constrain_High = 1.0f, const Enum_Filter_Fourier_Type &__Filter_Fourier_Type = Filter_Fourier_Type_LOWPASS, const float &__Frequency_Low = 0.0f, const float &__Frequency_High = FOURIER_FILTER_DEFAULT_SAMPLING_FREQUENCY / 2.0f, const float &__Sampling_Frequency = FOURIER_FILTER_DEFAULT_SAMPLING_FREQUENCY);
...
void Class_Filter_Fourier<Filter_Fourier_Order>::Init(const float &__Value_Constrain_Low, const float &__Value_Constrain_High, const Enum_Filter_Fourier_Type &__Filter_Fourier_Type, const float &__Frequency_Low, const float &__Frequency_High, const float &__Sampling_Frequency)
{
    ...
    case (Filter_Fourier_Type_LOWPASS):
    {
        ...
        {
            System_Function[i] = omega_low / PI * Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * omega_low);
        }
    ...
    case (Filter_Fourier_Type_HIGHPASS):
    {
        for (int i = 0; i < Filter_Fourier_Order + 1; i++)
        {
            System_Function[i] = Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * PI) - omega_high / PI * Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * omega_high);
        }
    ...
    case (Filter_Fourier_Type_BANDPASS):
    {
        for (int i = 0; i < Filter_Fourier_Order + 1; i++)
        {
            System_Function[i] = omega_high / PI * Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * omega_high) - omega_low / PI * Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * omega_low);
        }
    ...
    case (Filter_Fourier_Type_BANDSTOP):
    {
        for (int i = 0; i < Filter_Fourier_Order + 1; i++)
        {
            System_Function[i] = Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * PI) + omega_low / PI * Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * omega_low) - omega_high / PI * Math_Sinc(((float) (i) - Filter_Fourier_Order / 2.0f) * omega_high);
        }
 ...
 */
template<uint32_t Filter_Fourier_Order>
inline float Class_Filter_Fourier<Filter_Fourier_Order>::Get_Out() const
{
    return (Out);
}
 ...
 */
template<uint32_t Filter_Fourier_Order>
inline void Class_Filter_Fourier<Filter_Fourier_Order>::Set_Now(const float &__Now)
{
    float now_value;
